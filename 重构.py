...
def process_rolls():
    ...
    for folder in folders:
        ...
        # 搜集根目录和 images/ 里的遗留图片，统一归安到 raw/ 保障安全
        ...
        for img_name in raw_images:
            ...
            if not os.path.exists(dst_web):
                print(f"  └─ 生成微缩图: {img_name} -> {webp_name}")
...
.gallery img {{ width: 100%; display: block; cursor: pointer; background: #1a1a1a; transition: transform 0.15s ease, opacity 0.15s ease; box-shadow: 0 1px 4px rgba(0,0,0,0.4); user-select: none; }}
...
<style>
body {{ margin: 0; background: #111; color: #ddd; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; }}
.header {{ display: flex; justify-content: space-between; align-items: center; max-width: 720px; margin: 24px auto 0; padding: 0 16px; }}
...
.letters-btn {{ font-size: 12px; color: #aaa; text-decoration: none; border: 1px solid #333; padding: 5px 12px; border-radius: 4px; transition: all 0.2s; }}
.letters-btn:hover {{ color: #fff; border-color: #666; background: #1a1a1a; }}
.list {{ max-width: 720px; margin: 20px auto 0; padding: 0 10px 30px; }}
.roll {{ display: flex; justify-content: space-between; align-items: center; padding: 14px 10px; border-bottom: 1px solid #222; text-decoration: none; color: #ddd; }}
